import argparse
import os
import cv2
import random
import colorsys
import json
import numpy as np
import sys
from boxmot import BoostTrack
from pathlib import Path

sys.path.append('./')

# Initialize the tracker
tracker = BoostTrack(
    reid_weights=Path('models/osnet_x1_0_imagenet.pth'),
    device='cpu',
    half=False,
    det_thresh=0.3,
    max_age=100
)


# Initialize random seed for consistent colors
random.seed(100)
id_colors = {}

def get_color(track_id):
    if track_id not in id_colors:
        h = random.random()
        s, v = 1.0, 1.0
        r, g, b = colorsys.hsv_to_rgb(h, s, v)
        id_colors[track_id] = (int(b * 255), int(g * 255), int(r * 255))  # BGR for OpenCV
    return id_colors[track_id]

# Parse command-line arguments
parser = argparse.ArgumentParser(description='Track and update keypoints with IDs.')
parser.add_argument('--input_video', type=str, required=True, help='Path to input video file')
parser.add_argument('--input_json', type=str, required=True, help='Path to input keypoints JSON')
parser.add_argument('--output_video', type=str, required=True, help='Path to save output video')
parser.add_argument('--output_json', type=str, required=True, help='Path to save modified keypoints JSON')
args = parser.parse_args()

# Load keypoints
with open(args.input_json, 'r', encoding='utf-8') as f:
    kpts = json.load(f)

# Load video
cap = cv2.VideoCapture(args.input_video)
if not cap.isOpened():
    raise IOError("Cannot open video file")

# Get video dimensions
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
fps = cap.get(cv2.CAP_PROP_FPS)

# Output writer
out_dir = os.path.dirname(args.output_video)
if out_dir and not os.path.exists(out_dir):
    os.makedirs(out_dir, exist_ok=True)

fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter(args.output_video, fourcc, fps, (frame_width, frame_height))

frame_idx = 0
while True:
    ret, img = cap.read()
    if not ret or frame_idx >= len(kpts):
        break

    frame_dets = []
    for instance in kpts[frame_idx]['instances']:
        one_set = []
        one_set += instance['bbox'][0]
        one_set += [instance['bbox_score']]
        frame_dets += [one_set]

    dets = np.hstack([frame_dets, np.zeros((len(frame_dets), 1))]).astype(np.float32)
    tracked_objects = tracker.update(dets, img)

    new_instances = dict()
    for track in tracked_objects:
        x1, y1, x2, y2 = map(int, track[:4])
        track_id = int(track[4])
        color = get_color(track_id)
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
        cv2.putText(img, f'ID {track_id}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

        new_instances[track_id] = kpts[frame_idx]['instances'][int(track[7])]

    kpts[frame_idx]['instances'] = new_instances

    out.write(img)
    frame_idx += 1

cap.release()
out.release()
print("Video saved successfully.")

with open(args.output_json, 'w', encoding='utf-8') as f:
    json.dump(kpts, f, ensure_ascii=False, indent=4)
