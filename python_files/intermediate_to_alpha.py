...
import argparse
# ----------------------------
# ADD argparse to accept custom filenames
parser = argparse.ArgumentParser(description='Process tracking JSON and video files with custom filenames.')
parser.add_argument('--input_json', type=str, required=True, help='Path to the modified JSON input')
parser.add_argument('--input_video', type=str, required=True, help='Path to the input video file')
parser.add_argument('--output_json_prefix', type=str, required=True, help='Prefix path for output JSON files')
args = parser.parse_args()


with open(args.input_json, 'r') as f:  # changed
    rtm = json.load(f)
...
cap = cv2.VideoCapture(args.input_video)  # changed
if not cap.isOpened():
...
with open(f'{args.output_json_prefix}_toalpha_0.json', 'w') as f:  # changed
    json.dump(res, f, indent=4)

with open(f'{args.output_json_prefix}_toalpha_1.json', 'w') as f:  # changed
    json.dump(res1, f, indent=4)

with open(f'{args.output_json_prefix}_toalpha_2.json', 'w') as f:  # changed
